...
int main(int, char**)
{
	...
	VERTEX_BUFFER_DESC vbGridDesc = {};
	vbGridDesc.pData		 = reinterpret_cast<LPVOID>(vertices.data());
	vbGridDesc.StrideInBytes = sizeof(vertices[0]);
	vbGridDesc.StrideInBytes = sizeof(vertices[0]) * vertices.size();
	...
	UINT gridSize   = vbGridDesc.SizeInBytes;
	UINT posSize	= sizeof(positions);
	...
	//Internally, a resource is inserted for the vertex buffers
	//belonging to the vertices and and the grid
	...
	//Create/Insert positions
	dynamicHeap.InsertResource(device.GetDevice(),
							   (vbDesc.SizeInBytes + grid.GetBufferDesc().SizeInBytes),
							   bufferDesc,
							   D3D12_RESOURCE_STATE_COPY_DEST,
							   posResource.Get()); 
	...
	//Grid Transfer Data
	subResources[1].pData	  = vbGridDesc.pData;
	subResources[1].RowPitch   = vbGridDesc.SizeInBytes;
