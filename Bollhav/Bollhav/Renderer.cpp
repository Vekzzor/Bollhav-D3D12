...
void Renderer::init(const UINT wWidth, const UINT wHeight, HWND& wndHandle)
{
	m_width = wWidth;
	m_height = wHeight;
...
void Renderer::CreateDevice()
{
	...
	for (UINT adapterIndex = 0;; ++adapterIndex)
	{
		...
		if (SUCCEEDED(D3D12CreateDevice(adapter, D3D_FEATURE_LEVEL_12_1, __uuidof(m_device4), nullptr)))
		{
			break;
		}
		SafeRelease(&adapter);
	...
	if (adapter)
	{
		...
		if (SUCCEEDED(hr = D3D12CreateDevice(adapter, D3D_FEATURE_LEVEL_12_1, IID_PPV_ARGS(&m_device4))))
		{
...
void Renderer::CreateSwapChainAndCommandIterface(HWND& whand)
{
	...
	hr = m_device4->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_commandAllocator));
...
void Renderer::CreateRenderTarget()
{
	...
	m_renderTargetDescSize = m_device4->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

	//The pointer starts at the start of the heap for the render target views. 
	D3D12_CPU_DESCRIPTOR_HANDLE cpuDescHand = m_renderTargetsHeap->GetCPUDescriptorHandleForHeapStart(); 
...
void Renderer::CreateRootSignature()
{
	...
	dtRanges[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;  //Automatic append between slots in the heap. 
